...
import mobi.mateam.alarma.view.tools.BitmapUtil;
import mobi.mateam.alarma.view.tools.PixelUtil;
...
public class RangeSeekBar<T extends Number> extends android.support.v7.widget.AppCompatImageView {
  ...
  @SuppressWarnings("unchecked") private T extractNumericValueFromAttributes(TypedArray a, int attribute, int defaultValue) {
  ...
  private void init(Context context, AttributeSet attrs) {
    float barHeight;
    int thumbNormal = R.drawable.seek_normal;
    int thumbPressed = R.drawable.seek_pressed;
    int thumbDisabled = R.drawable.seek_disabled;
    ...
    mTextOffset = !mShowTextAboveThumbs ? 0 : this.mTextSize + PixelUtil.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP) + this.mDistanceToTop;

    mRect = new RectF(padding, mTextOffset + mThumbHalfHeight - barHeight / 2, getWidth() - padding, mTextOffset + mThumbHalfHeight + barHeight / 2);
  ...
  @SuppressWarnings("unused") public void resetSelectedValues() {
    setSelectedMinValue(absoluteMinValue);
    setSelectedMaxValue(absoluteMaxValue);
  }

  @SuppressWarnings("unused") public boolean isNotifyWhileDragging() {
   ...
   */
  @SuppressWarnings("unused") public void setNotifyWhileDragging(boolean flag) {
   ...
   */
  @SuppressWarnings("unused") public void setOnRangeSeekBarChangeListener(OnRangeSeekBarChangeListener<T> listener) {
   ...
   */
  @SuppressWarnings("unused") public void setThumbShadowPath(Path thumbShadowPath) {
  ...
  @Override protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int height = thumbImage.getHeight() + (!mShowTextAboveThumbs ? 0 : PixelUtil.dpToPx(getContext(), HEIGHT_IN_DP)) + (mThumbShadow ?
        mThumbShadowYOffset
            + mThumbShadowBlur : 0);
   ...
   */
  @Override protected synchronized void onDraw(@NonNull Canvas canvas) {
    super.onDraw(canvas);
    ...
    boolean selectedValuesAreDefault = (getSelectedMinValue().equals(getAbsoluteMinValue()) && getSelectedMaxValue().equals(getAbsoluteMaxValue()));

    int colorToUseForButtonsAndHighlightedLine =
        !mAlwaysActive && !mActivateOnDefaultValues && selectedValuesAreDefault ? mDefaultColor : // default values
  ...
  private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
    ...
    canvas.drawBitmap(buttonToDraw, screenCoord - mThumbHalfWidth, mTextOffset, paint);
  }
   ...
   */
  @SuppressWarnings("unchecked") private T normalizedToValue(double normalized) {
