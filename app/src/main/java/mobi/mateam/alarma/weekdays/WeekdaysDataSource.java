...
public class WeekdaysDataSource implements Parcelable {
  ...
  public static final Creator<WeekdaysDataSource> CREATOR = new Creator<WeekdaysDataSource>() {
  ...
  @IdRes private int mAttachId = R.id.weekdays_recycler_view;
  @LayoutRes private int mWeekdayLayoutId = -1;//R.layout.weekdays_image_view;
  @IdRes private int mWeekdayViewId = -1;//R.id.weekday_view;
  @ColorInt private int mBackgroundColor = Color.TRANSPARENT;
  @ColorInt private int mTextColorSelected;
  @ColorInt private int mTextColorUnselected;
  @ColorInt private int mSelectedColor;
  @ColorInt private int mUnselectedColor;
  ...
  public WeekdaysDataSource(@NonNull Fragment context, @IdRes int attacherId) {
    mContext = (AppCompatActivity) context.getActivity();
  ...
  @UiThread public static WeekdaysDataSource restoreState(@NonNull String tag, Bundle source, AppCompatActivity context, Callback callback,
      TextDrawableListener drawableListener) {
    if (source == null || !source.containsKey("weekdays_state_" + tag)) return null;
  ...
  @UiThread public WeekdaysDataSource setSelectedDays(int... indexes) {
    if (indexes.length > getDays().size()) indexes = Arrays.copyOf(indexes, getDays().size());
    for (Integer index : indexes) {
      mIsAllDaysSelected = false;
      if (index <= getDays().size()) getSelectedDays().put(index, true);
  ...
  @UiThread public WeekdaysDataSource reset(Context context) {
    ...
    mSelectedColor =
        WeekdaysUtil.resolveColor(context, R.attr.weekdays_selected_color, WeekdaysUtil.resolveColor(context, R.attr.colorAccent, Color.RED));
    mUnselectedColor =
        WeekdaysUtil.resolveColor(context, R.attr.weekdays_unselected_color, WeekdaysUtil.resolveColor(context, R.attr.colorPrimary, Color.GRAY));
  ...
  @UiThread public WeekdaysDataSource setFirstDayOfWeek(@IntRange(from = Calendar.SUNDAY, to = Calendar.MONDAY) int firstDayOfWeek) {
  ...
  @UiThread public WeekdaysDataSource setFillWidth(boolean mFillView) {
  ...
  @UiThread public WeekdaysDataSource setNestedScrollEnable(boolean enable) {
    this.mNestedScrollEnable = enable;
    if (mRecyclerView != null) mRecyclerView.setNestedScrollingEnabled(enable);
    return this;
  }

  @UiThread public WeekdaysDataSource setDrawableType(int mType) {
  ...
  public WeekdaysDataSource setFontTypeFace(Typeface fontTypeFace) {
    ...
    if (getDrawableProvider() != null) getDrawableProvider().setFontTypeFace(fontTypeFace);
    return this;
  }

  @UiThread public WeekdaysDataSource setNumberOfLetters(int numberOfLetters) {
  ...
  @UiThread public WeekdaysDataSource setFontBaseSize(int fontBaseSize) {
    if (getDrawableProvider() != null) getDrawableProvider().setFontBaseSize(fontBaseSize);
    return this;
  }

  @UiThread public WeekdaysDataSource setSelectedDays(boolean... days) {
    if (days.length > getDays().size()) days = Arrays.copyOf(days, getDays().size());
  ...
  @UiThread public WeekdaysDataSource start(@Nullable Callback callback) {
    mCallback = callback;
    invalidateVisibility(attach());
    return this;
  }

  @UiThread public WeekdaysDataSource setBackgroundColor(@ColorInt int color) {
    mBackgroundColor = color;
    if (mRecyclerView != null) mRecyclerView.setBackgroundColor(color);
    return this;
  }

  @UiThread public WeekdaysDataSource setBackgroundColorRes(@ColorRes int colorRes) {
    return setBackgroundColor(ContextCompat.getColor(mContext, colorRes));
  }

  @UiThread public WeekdaysDataSource setBackgroundColorAttr(@AttrRes int colorAttr) {
    return setBackgroundColor(WeekdaysUtil.resolveColor(mContext, colorAttr, 0));
  }

  @UiThread public WeekdaysDataSource setTextColorSelected(@ColorInt int color) {
    mTextColorSelected = color;
    if (getDrawableProvider() != null) getDrawableProvider().setTextColorSelected(color);
    return this;
  }

  @UiThread public WeekdaysDataSource setTextColorSelectedRes(@ColorRes int colorRes) {
    return setTextColorSelected(ContextCompat.getColor(mContext, colorRes));
  }

  @UiThread public WeekdaysDataSource setTextColorSelectedAttr(@AttrRes int colorAttr) {
    return setTextColorSelected(WeekdaysUtil.resolveColor(mContext, colorAttr, 0));
  }

  @UiThread public WeekdaysDataSource setTextColorUnselected(@ColorInt int color) {
    mTextColorUnselected = color;
    if (getDrawableProvider() != null) getDrawableProvider().setTextColorUnselected(color);
    return this;
  }

  @UiThread public WeekdaysDataSource setTextColorUnselectedRes(@ColorRes int colorRes) {
    return setTextColorUnselected(ContextCompat.getColor(mContext, colorRes));
  }

  @UiThread public WeekdaysDataSource setTextColorUnselectedAttr(@AttrRes int colorAttr) {
    return setTextColorUnselected(WeekdaysUtil.resolveColor(mContext, colorAttr, 0));
  }

  @UiThread public WeekdaysDataSource setSelectedColor(@ColorInt int color) {
    mSelectedColor = color;
    if (getDrawableProvider() != null) getDrawableProvider().setSelectedColor(color);
    return this;
  }

  @UiThread public WeekdaysDataSource setSelectedColorRes(@ColorRes int colorRes) {
    return setSelectedColor(ContextCompat.getColor(mContext, colorRes));
  }

  @UiThread public WeekdaysDataSource setSelectedColorAttr(@AttrRes int colorAttr) {
    return setSelectedColor(WeekdaysUtil.resolveColor(mContext, colorAttr, 0));
  }

  @UiThread public WeekdaysDataSource setUnselectedColor(@ColorInt int color) {
    mUnselectedColor = color;
    if (getDrawableProvider() != null) getDrawableProvider().setUnselectedColor(color);
    return this;
  }

  @UiThread public WeekdaysDataSource setUnselectedColorRes(@ColorRes int colorRes) {
    return setUnselectedColor(ContextCompat.getColor(mContext, colorRes));
  }

  @UiThread public WeekdaysDataSource setUnselectedColorAttr(@AttrRes int colorAttr) {
    return setUnselectedColor(WeekdaysUtil.resolveColor(mContext, colorAttr, 0));
  }

  @UiThread public WeekdaysDataSource setViewMarginRes(@DimenRes int marginRes) {
    return setViewMargin((int) mContext.getResources().getDimension(marginRes));
  }

  @UiThread public WeekdaysDataSource setViewMarginAttr(@AttrRes int marginAttr) {
    return setViewMargin(WeekdaysUtil.resolveInt(mContext, marginAttr, 0));
  }

  @UiThread public WeekdaysDataSource setViewWidthRes(@DimenRes int widthRes) {
    return setViewWidth((int) mContext.getResources().getDimension(widthRes));
  }

  @UiThread public WeekdaysDataSource setViewWidthAttr(@AttrRes int widthAttr) {
    return setViewWidth(WeekdaysUtil.resolveInt(mContext, widthAttr, 0));
  }

  @UiThread public WeekdaysDataSource setViewHeightRes(@DimenRes int heigthRes) {
    return setViewHeight((int) mContext.getResources().getDimension(heigthRes));
  }

  @UiThread public WeekdaysDataSource setViewHeightAttr(@AttrRes int heightAttr) {
    return setViewHeight(WeekdaysUtil.resolveInt(mContext, heightAttr, 0));
  }

  @UiThread public WeekdaysDataSource setLayoutPaddingRes(@DimenRes int layoutPaddingRes) {
    return setLayoutPadding((int) mContext.getResources().getDimension(layoutPaddingRes));
  }

  @UiThread public WeekdaysDataSource setLayoutPaddingAttr(@AttrRes int layoutPaddingAttr) {
    return setLayoutPadding(WeekdaysUtil.resolveInt(mContext, layoutPaddingAttr, 0));
  }

  @UiThread public WeekdaysDataSource setViewParams(int width, int height, int margin, int gravity, int layoutPadding) {
    setViewWidth(width);
  ...
  @UiThread public WeekdaysDataSource setViewParamsRes(@DimenRes int widthRes, @DimenRes int heightRes, @DimenRes int marginRes, int gravity,
      @DimenRes int layoutPaddingRes) {
  ...
  @UiThread public WeekdaysDataSource setViewParamsAttr(@AttrRes int widthAttr, @AttrRes int heightAttr, @AttrRes int marginAttr, int gravity,
      @AttrRes int layoutPaddingAttr) {
  ...
  @UiThread public WeekdaysDataSource setViewMargin(int mViewMargin) {
  ...
  @UiThread public WeekdaysDataSource setViewWidth(int mViewWidth) {
  ...
  @UiThread public WeekdaysDataSource setViewHeight(int mViewHeight) {
   ...
   * @return WeekdaysDataSource
   */
  @UiThread public WeekdaysDataSource setViewGravity(int gravity) {
  ...
  @UiThread public WeekdaysDataSource setLayoutPadding(int layoutPadding) {
    this.mLayoutPadding = layoutPadding;
    return this;
  }

  @Nullable public View getWeekdaysRecycleView() {
  ...
  private void setSelected(int index, boolean selected, boolean notify) {
    ...
    if (item != null) {
      ...
      if (notify && mRecyclerView != null) mRecyclerView.getAdapter().notifyItemChanged(index);
    }
  }
  ...
  @IdRes public int getWeekdayViewId() {
  ...
  private WeekdaysDataItem itemFromType(int position, int calendarDayId, String label, boolean selected) {
    return new WeekdaysDataItem.Builder().setLayoutParentId(mAttachId)
  ...
  private Drawable getDrawableFromType(int calendarDayId, int textDrawableType, String label, boolean selected) {
    ...
    if (subStringLength > mNumberOfLetters) subStringLength = mNumberOfLetters;
  ...
  protected RecyclerView.Adapter<WeekdaysAdapter.ViewHolder> createAdapter() {
    return new WeekdaysAdapter(getWeekdayLayoutId(), getWeekdayViewId(), getWeekdaysItems(), getFillWidth(), getViewWidth(), getViewHeight(),
        getViewMargin(), getViewGravity(), getLayoutPadding()) {
  ...
  private boolean attach() {
    ...
    if (mRecyclerView != null) {
      ...
      if (mTextDrawableType == WeekdaysDrawableProvider.MW_MULTIPLE_LETTERS && mNumberOfLetters < 2) mNumberOfLetters = 2;
      ...
      for (Map.Entry<Integer, String> map : getDays().entrySet()) {
        ...
        if (!TextUtils.isEmpty(day)) {
          ...
          if (getWeekdaysCount() == position) {
            ...
            getWeekdaysItems().set(position, item);
          }
  ...
  @UiThread public void saveState(@NonNull String tag, Bundle dest) {
