...
public class WeekdaysDrawableProvider {
  ...
  private Typeface getFontTypeFace() {
    if (fontTypeFace == null) fontTypeFace = Typeface.create("sans-serif-light", Typeface.NORMAL);
    if (fontTypeFace == null) fontTypeFace = Typeface.SANS_SERIF;
    return fontTypeFace;
  }
  ...
  public Drawable getRectWithCustomSize(Context context, String leftText, String rightText, boolean selected) {
    ...
    Drawable[] layerList = {
        new InsetDrawable(left, 0, 0, toPx(context, 31), 0), new InsetDrawable(right, toPx(context, 31), 0, 0, 0)
    };
