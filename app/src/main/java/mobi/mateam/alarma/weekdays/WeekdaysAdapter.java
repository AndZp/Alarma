...
public class WeekdaysAdapter extends RecyclerView.Adapter<WeekdaysAdapter.ViewHolder> {
  ...
  public WeekdaysAdapter(@LayoutRes int layout_id, @IdRes int view_id, ArrayList<WeekdaysDataItem> items, boolean fillWidth, int viewWidth,
      int viewHeight, int viewMargin, int viewGravity, int layoutPaddingParent
  ...
  @Override public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    if (view == null) {
      ...
      view.setLayoutParams(
          new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.MATCH_PARENT, Gravity.CENTER));
    ...
    if (h.mWeekdayView == null && h.mViewHolder instanceof ViewGroup) {
      ...
      FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(mWidthPx, mHeightPx, layout_gravity);
      params.setMargins(margin, 0, margin, 0);
      ((ViewGroup) h.mViewHolder).addView(h.mWeekdayView, params);
    }

    if (h.mWeekdayView != null) {
      if (h.mWeekdayView instanceof FloatingActionButton) h.mWeekdayView.setPadding(0, 0, 0, 0);
      h.mWeekdayView.setFocusable(true);
      h.mWeekdayView.setClickable(false);
    }
    if (isFillWidth) h.mViewHolder.setMinimumWidth(Math.round(parent.getWidth() / getItemCount()));
