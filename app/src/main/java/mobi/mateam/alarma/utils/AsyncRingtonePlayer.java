...
import mobi.mateam.alarma.alarm.model.TimeModel;
...
public final class AsyncRingtonePlayer {
   ...
   */
  @SuppressLint("HandlerLeak") private Handler getNewHandler() {
  ...
  private void checkAsyncRingtonePlayerThread() {
    if (Looper.myLooper() != mHandler.getLooper()) {
      Timber.e("Must be on the AsyncRingtonePlayer thread!", new IllegalStateException());
    }
  }
  ...
  private static float computeVolume(long currentTime, long stopTime, long duration) {
    ...
    Timber.v("Ringtone crescendo %,.2f%% complete (scalar: %f, volume: %f dB)", fractionComplete * 100, volume, gain);
  ...
  private class MediaPlayerPlaybackDelegate implements PlaybackDelegate {
     ...
     */
    @Override public boolean play(final Context context, Uri ringtoneUri, long crescendoDuration) {
      checkAsyncRingtonePlayerThread();
      mCrescendoDuration = crescendoDuration;

      Timber.i("Play ringtone via android.media.MediaPlayer.");
      ...
      if (alarmNoise == null) {
        ...
        Timber.v("Using default alarm: " + alarmNoise.toString());
      }

      mMediaPlayer = new MediaPlayer();
      mMediaPlayer.setOnErrorListener(new MediaPlayer.OnErrorListener() {
        @Override public boolean onError(MediaPlayer mp, int what, int extra) {
          Timber.e("Error occurred while playing audio. Stopping AlarmKlaxon.");
          stop(context);
          return true;
        }
      });
      ...
      try {
        ...
        Timber.e("Using the fallback ringtone, could not play " + alarmNoise, t);
        ...
        try {
          ...
          Timber.e("Failed to play fallback ringtone", t2);
        }
      }
    ...
    private boolean startPlayback(boolean inTelephoneCall) throws IOException {
      ...
      if (inTelephoneCall) {
        Timber.v("Using the in-call alarm");
        mMediaPlayer.setVolume(IN_CALL_VOLUME, IN_CALL_VOLUME);
      } else if (mCrescendoDuration > 0) {
        mMediaPlayer.setVolume(0, 0);

        // Compute the time at which the crescendo will stop.
        mCrescendoStopTime = new TimeModel(mContext).elapsedRealtime() + mCrescendoDuration;
    ...
    @Override public void stop(Context context) {
      ...
      Timber.i("Stop ringtone via android.media.MediaPlayer.");
     ...
     */
    @Override public boolean adjustVolume(Context context) {
      checkAsyncRingtonePlayerThread();
      ...
      final long currentTime = new TimeModel(mContext).currentTimeMillis();
      ...
      Timber.i("MediaPlayer volume set to " + volume);
  ...
  private class RingtonePlaybackDelegate implements PlaybackDelegate {
    ...
    private RingtonePlaybackDelegate() {
      try {
        ...
        Timber.e("Unable to locate method: Ringtone.setVolume(float).", nsme);
      }

      try {
        mSetLoopingMethod = Ringtone.class.getDeclaredMethod("setLooping", boolean.class);
      } catch (NoSuchMethodException nsme) {
        Timber.e("Unable to locate method: Ringtone.setLooping(boolean).", nsme);
      }
    }
     ...
     * Starts the actual playback of the ringtone. Executes on ringtone-thread.
     */
    @Override public boolean play(Context context, Uri ringtoneUri, long crescendoDuration) {
      checkAsyncRingtonePlayerThread();
      mCrescendoDuration = crescendoDuration;

      Timber.i("Play ringtone via android.media.Ringtone.");
      ...
      try {
        ...
        Timber.e("Unable to turn looping on for android.media.Ringtone", e);
      ...
      if (mRingtone == null) {
        Timber.i("Unable to locate alarm ringtone, using internal fallback ringtone.");
      ...
      try {
        ...
        Timber.e("Using the fallback ringtone, could not play " + ringtoneUri, t);
    ...
    private boolean startPlayback(boolean inTelephoneCall) {
      ...
      if (inTelephoneCall) {
        ...
        // Compute the time at which the crescendo will stop.
        mCrescendoStopTime = new TimeModel(mContext).currentTimeMillis() + mCrescendoDuration;
        scheduleVolumeAdjustment = true;
      }
    ...
    private void setRingtoneVolume(float volume) {
      try {
        ...
        Timber.e("Unable to set volume for android.media.Ringtone", e);
      }
    }
    ...
    @Override public void stop(Context context) {
      ...
      Timber.i("Stop ringtone via android.media.Ringtone.");

      mCrescendoDuration = 0;
      mCrescendoStopTime = 0;

      if (mRingtone != null && mRingtone.isPlaying()) {
        Timber.d("Ringtone.stop() invoked.");
        mRingtone.stop();
      }
